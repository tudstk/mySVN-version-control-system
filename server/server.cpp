...
void listFiles(const char *dir_name, int sd)
{
    ...
    struct dirent *entry;
    int bWritten;
    char directoryMsg[200], fileMsg[200];
    memset(directoryMsg, 0, 200);
    memset(fileMsg, 0, 200);
    ...
    while ((entry = readdir(dir)) != NULL)
    {
        if (entry->d_type == DT_REG)
        {
            ...
            // memset(fileMsg, 0, 200);
            // strcpy(fileMsg, "file: ");
            // strcat(fileMsg, entry->d_name);
            // printf("%s\n", fileMsg);
            if ((bWritten = (write(sd, entry->d_name, strlen(entry->d_name)))) <= 0)
            {
                perror("[Server] Eroare la write din functie!\n");
            }
        }
...
int compareFiles(const char *filePath_1, const char *filePath_2)
{
    ...
    // // in cazul in care un fisier are mai multe linii decat celalalt:
    // // while-ul de mai sus se va opri daca pana la linia x fisierele erau la fel si ar returna true chiar dac
    // char *bonusLine_1 = fgets(tempArray1, 1000, file_1);
    // char *bonusLine_2 = fgets(tempArray2, 1000, file_2);

    // if (bonusLine_1 != NULL || bonusLine_2 != NULL) {
    //     areEqual = 0;
    // }
    fclose(file_1);
    fclose(file_2);
...
void raspunde(void *arg)
{
    ...
    do
    {
        ...
        if (strcmp(processCommand, "login") == 0)
        {
            ...
            else
            {
                ...
                do
                {
                    ...
                    {
                        printf("am primit list\n");
                        listFiles("./version_0", tdL.cl);
                        return;
                    }
                    ...
                    else if (strstr(accountCommand, "exit"))
                    {
                        ...
                        // if (read(tdL.cl, exitSignal, 50) <= 0)
                        // {
                        //     perror("[Server] Eroare la read() in server (1).\n");
                        // }
                        // if (strstr(exitSignal, "semnal"))
                        // {
                        //     char ack[50];
                        //     memset(ack, 0, 50);
                        //     printf("[Server] User-ul '%s' s-a deconectat de la server.\n", inputUser);
                        //     strcpy(ack, "ai permisiunea sa te inchizi.");
                        //     if (write(tdL.cl, ack, strlen(ack)) <= 0)
                        //     {
                        //         perror("[Server] Delete write error.\n");
                        //     }
                        // }
        ...
        else if (strcmp(processCommand, "signup") == 0)
        {
            ...
            {
                int sqlAction = sqlite3_prepare_v2(db, "INSERT INTO users (username, password, permission) VALUES(?, ?, ?)", -1, &stmt, 0);

                sqlite3_bind_text(stmt, 1, inputUser, -1, SQLITE_STATIC);
                sqlite3_bind_text(stmt, 2, inputPasswd, -1, SQLITE_STATIC);
                sqlite3_bind_text(stmt, 3, "true", -1, SQLITE_STATIC);
